...
import com.deras.id.databinding.HistoryCardBinding
    ...
    class MyViewHolder(private val binding: HistoryCardBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(history: HistoryEntity) {
            ...
            binding.root.setOnClickListener {
                val intent = Intent(binding.root.context, ResultActivity::class.java).apply {
                    putExtra(ResultActivity.EXTRA_RESULT, history.result)
                    putExtra(ResultActivity.EXTRA_CREATED_AT, history.date)
                    putExtra(ResultActivity.EXTRA_SUGGESTION, history.suggestion)
                    putExtra(ResultActivity.EXTRA_EXPLANATION, history.explanation)
                }
